...
namespace bitrl
{
    ...
    namespace sensors
    {
        ...
        public:
            ...
            void set_init_covariance(DynMat<real_t>& cov_mat){P_ = cov_mat;}
            void set_process_noise(DynMat<real_t>& noise_mat){Q_ = noise_mat;}
            ...
            /// @param dt
            void predict(real_t dt, const std::string& sensor_message, const topic_type& topic);

            void update(real_t dr, const std::string& sensor_message, const topic_type& topic);

        private:

            RealVec x_; // state vector
            DynMat<real_t> P_; // covariance
            DynMat<real_t> Q_; // process noise
            DynMat<real_t> K_; // gain matrix

            const motion_model_type* motion_model_;
            ...
            std::unordered_map<topic_type, DynMat<real_t>> sensor_noise_;
        ...
        EKFSensorFusion<MotionModelType, ObservationModelType>::step(real_t dt)
        {
            ...
            for (auto& sensor : sensors_)
            {
                ...
                {
                    predict(dt, sensor_message.value(), sensor.first);
                                                                         ...
                                                                         const topic_type& topic)
        {
            ...
            /// motion model
            motion_model_->evaluate(dt, sensor_message, topic);
                                                                         ...
                                                                         const topic_type& topic)
        {
            ...
            // auto& R = (*this)["R"];
            //
            // auto zpred = observation_model_->evaluate(dt, sensor_message, topic);
            //
