...
void EpuckRobot::activate_motor(uint_t motor_id, real_t init_velocity)
{
...
void EpuckRobot::activate_position_sensor(uint_t sensor_id, uint_t time_step)
{
...
void EpuckRobot::activate_proximity_sensor(const std::string &name, uint_t time_step)
{
...
real_t EpuckRobot::get_distance_value_from_sensor(const std::string &sensor_name) const
{
...
std::vector<real_t> EpuckRobot::read_distance_sensors() const
{
...
RBTranslation EpuckRobot::get_position() const
{

    // const real_t *position = robot_ -> getFromDef("e-puck") -> getPosition(); //getSelf() ->
    // getPosition(); getFromDef("e-puck") -> getPosition();
    const real_t *position =
        robot_->getSelf()->getField("translation")->getSFVec3f(); // getPosition();
    return {position[0], position[1], position[2]};
}

RBRotation EpuckRobot::get_rotation() const
{
...
EpuckOdometry EpuckRobot::compute_odometry() const
{
...
std::ostream &EpuckOdometry::print(std::ostream &out) const noexcept
{
