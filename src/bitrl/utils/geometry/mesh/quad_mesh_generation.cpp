...
#include "bitrl/utils/geometry/mesh/element_mesh_iterator.h"
#include "bitrl/utils/geometry/mesh/element_type.h"
#include "bitrl/utils/geometry/mesh/face_element.h"
#include "bitrl/utils/geometry/mesh/face_mesh_iterator.h"
...
#include "bitrl/utils/geometry/mesh/mesh_predicates.h"
...
namespace
{
inline uint_t idx(uint_t nx, uint_t i, uint_t j) { return i + j * (nx + 1); }

uint_t find_neighbor_id(const Element<2> &e, uint_t f,
                        const std::vector<MeshConnectivity> &node_connections)
{
    ...
    std::set_intersection(v0.connectivity_begin(), v0.connectivity_end(), v1.connectivity_begin(),
                          v1.connectivity_end(), std::back_inserter(common));

    // remove the id of the element we work on
    common.erase(std::remove(common.begin(), common.end(), e.get_id()), common.end());
...
void build_rectangular_mesh_vertices(const GeomPoint<2> &lower_point,
                                     const GeomPoint<2> &upper_point, uint_t Nx, uint_t Ny,
                                     Mesh<2> &mesh)
{
...
void build_rectangular_mesh_elements(uint_t Nx, uint_t Ny, uint_t n_nodes_per_elem, Mesh<2> &mesh,
                                     std::vector<MeshConnectivity> &vertex_connections)
{
...
void build_quad_mesh_element_element_connectivity(
    Mesh<2> &m, const std::vector<MeshConnectivity> &vertices_connections)
{
    ...
    for (; e_it != e_it_end; ++e_it)
    {
        ...
        for (uint_t face = 0; face < element->n_faces(); ++face)
        {
            ...
            if (neigh_id == bitrl::consts::INVALID_ID)
            {
    ...
    for (; e_it != e_it_end; ++e_it)
    {
        ...
        for (uint_t neigh = 0; neigh < element->n_neighbors(); ++neigh)
        {
            ...
            if (element->neighbor_ptr(neigh) == nullptr)
            {
                ...
                FaceElement<2, 1> *face =
                    m.create_face(ElementType::sub_type::EDGE2, face_counter++);
            ...
            else
            {
                ...
                if (element->get_id() < element->neighbor_ptr(neigh)->get_id())
                {
                    ...
                    if (neigh_idx == bitrl::consts::INVALID_ID)
                    {
...
void build_quad_mesh(Mesh<2> &mesh, uint_t nx, uint_t ny, const GeomPoint<2, real_t> &lower_point,
