...
inline MeshConnectivity::MeshConnectivity() : connections_() {}

inline MeshConnectivity::MeshConnectivity(uint_t n) : connections_(n, bitrl::consts::INVALID_ID) {}

inline MeshConnectivity::MeshConnectivity(const std::vector<uint_t> &conn) : connections_(conn) {}

inline MeshConnectivity::MeshConnectivity(const MeshConnectivity &o) : connections_(o.connections_)
{
}
inline MeshConnectivity &MeshConnectivity::operator=(const MeshConnectivity &o)
{
...
inline void MeshConnectivity::assign_connections(const std::vector<uint_t> &connections)
{

    connections_.assign(connections.begin(), connections.end());
}

inline std::ostream &MeshConnectivity::print_connectivity_info(std::ostream &out) const
{
    std::copy(connections_.begin(), connections_.end(), std::ostream_iterator<uint_t>(out, "\n"));
    return out;
}

inline uint_t MeshConnectivity::connection_idx(uint_t i) const
{
    if (i >= connections_.size())
    {
        throw std::logic_error("Invalid connection index: " + std::to_string(i) + " not in [0, " +
...
inline std::ostream &operator<<(std::ostream &out, const MeshConnectivity &connectivity)
{
