...
class CSVMeshWriter
{
  ...
  public:
    ...
    /// \brief Constructor
    ///
    CSVMeshWriter(const std::string &filename,
    ...
    ///
    template <typename T> void write_row(const std::vector<T> &vals);

    ///
    /// \brief Write the given Vec as a row
    ///
    template <typename T> void write_row(const DynVec<T> &vals);

    ///
    /// \brief Write the given tuple as a row
    ///
    template <typename... T> void write_row(const std::tuple<T...> &row);
    ...
    ///
    template <typename MeshTp> void write_mesh_nodes(const MeshTp &mesh);
...
inline void CSVMeshWriter::write_column_names(const std::vector<std::string> &col_names,
                                              bool write_header)
{
...
inline void CSVMeshWriter::write_column_names(const std::vector<std::string_view> &col_names,
                                              bool write_header)
{
...
inline void
CSVMeshWriter::write_column_names(const std::initializer_list<std::string_view> &col_names,
                                  bool write_header)
{
...
template <typename T> void CSVMeshWriter::write_row(const std::vector<T> &vals)
{
    writer_.write_row(vals);
}

template <typename T> void CSVMeshWriter::write_row(const DynVec<T> &vals)
{

    writer_.write_row(vals);
}

template <typename MeshTp> void CSVMeshWriter::write_mesh_nodes(const MeshTp &mesh)
{
...
template <typename... T> void CSVMeshWriter::write_row(const std::tuple<T...> &row)
{
