...
class GymWalk final : public EnvBase<TimeStep<uint_t>, ScalarDiscreteEnv<state_size, 2, 0, 0>>
{
  public:
    ///
    /// \brief name
...
GymWalk<state_size>::GymWalk(network::RESTApiServerWrapper &api_server)
    : EnvBase<TimeStep<uint_t>, ScalarDiscreteEnv<state_size, 2, 0, 0>>(GymWalk<state_size>::name),
...
GymWalk<state_size>::GymWalk(const GymWalk<state_size> &other)
    : EnvBase<TimeStep<uint_t>, ScalarDiscreteEnv<state_size, 2, 0, 0>>(other),
...
GymWalk<state_size>::create_time_step_from_response_(const nlohmann::json &response) const
{
    ...
    return GymWalk::time_step_type(time_step_type_from_int(step_type), reward, observation,
...
template <uint_t state_size> bool GymWalk<state_size>::close()
{
