...
Pendulum::create_time_step_from_response_(const nlohmann::json &response) const
{
    ...
    return Pendulum::time_step_type(TimeStepEnumUtils::time_step_type_from_int(step_type), reward,
...
Pendulum::Pendulum(network::RESTRLEnvClient &api_server)
    : GymnasiumEnvBase<TimeStep<std::vector<real_t>>,
                       ContinuousVectorStateContinuousScalarBoundedActionEnv<
                           3, 1, RealRange<-2.0, 2.0>, 0, real_t>>(api_server, Pendulum::name)
{
...
Pendulum::Pendulum(const Pendulum &other)
    : GymnasiumEnvBase<TimeStep<std::vector<real_t>>,
                       ContinuousVectorStateContinuousScalarBoundedActionEnv<
                           3, 1, RealRange<-2.0, 2.0>, 0, real_t>>(other)
{
}

void Pendulum::make(const std::string &version,
...
Pendulum::time_step_type Pendulum::step(const action_type &action)
{
