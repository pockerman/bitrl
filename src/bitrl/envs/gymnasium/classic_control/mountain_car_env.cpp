...
MountainCar::MountainCar(network::RESTRLEnvClient &api_server)
    : GymnasiumEnvBase<TimeStep<std::vector<real_t>>,
                       ContinuousVectorStateDiscreteActionEnv<3, 2, 0, real_t>>(api_server,
                                                                                MountainCar::name)
{
...
MountainCar::MountainCar(const MountainCar &other)
    : GymnasiumEnvBase<TimeStep<std::vector<real_t>>,
                       ContinuousVectorStateDiscreteActionEnv<3, 2, 0, real_t>>(other)
{
}

void MountainCar::make(const std::string &version,
...
MountainCar::time_step_type MountainCar::step(const action_type &action)
{
