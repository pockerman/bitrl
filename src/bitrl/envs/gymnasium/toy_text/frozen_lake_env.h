 ...
 *  Frozen lake involves crossing a frozen lake from Start(S) to goal(G) without falling into any
 * holes(H). The agent may not always move in the intended direction due to the slippery nature of
 * the frozen lake
 *
 ...
 */
template <uint_t side_size> struct frozenlake_state_size;
    ...
    : public ToyTextEnvBase<TimeStep<uint_t>, frozenlake_state_size<side_size>::size, 3>
{
  public:
    ...
    /// \brief The base type
    ///
    typedef typename ToyTextEnvBase<TimeStep<uint_t>, frozenlake_state_size<side_size>::size,
