...
template <uint_t side_size> const std::string FrozenLake<side_size>::name = "FrozenLake";
...
FrozenLake<side_size>::FrozenLake(network::RESTRLEnvClient &api_server)
    : ToyTextEnvBase<TimeStep<uint_t>, frozenlake_state_size<side_size>::size, 3>(
          api_server, FrozenLake<side_size>::name)
{
...
FrozenLake<side_size>::FrozenLake(const FrozenLake<side_size> &other)
    : ToyTextEnvBase<TimeStep<uint_t>, frozenlake_state_size<side_size>::size, 3>(other)
{
}
...
FrozenLake<side_size>::create_time_step_from_response_(const nlohmann::json &response) const
{
    ...
    auto time_step = FrozenLake<side_size>::time_step_type(
        step_type_val, reward, observation, discount, std::unordered_map<std::string, std::any>());

    return time_step;
}

template <uint_t side_size> bool FrozenLake<side_size>::is_slippery() const
{
...
template <uint_t side_size>
void FrozenLake<side_size>::make(const std::string &version,
