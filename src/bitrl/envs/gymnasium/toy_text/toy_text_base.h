    ...
    : public GymnasiumEnvBase<TimeStepType, ScalarDiscreteEnv<state_end, action_end, 0, 0>>
{
  public:
    ...
    /// \brief The base_type
    ///
    typedef typename GymnasiumEnvBase<
        TimeStepType, ScalarDiscreteEnv<state_end, action_end, 0, 0>>::base_type base_type;
...
template <typename TimeStepType, uint_t state_end, uint_t action_end>
ToyTextEnvBase<TimeStepType, state_end, action_end>::ToyTextEnvBase(
    network::RESTRLEnvClient &api_server, const std::string &name)
    : GymnasiumEnvBase<TimeStepType, ScalarDiscreteEnv<state_end, action_end>>(api_server, name)
{
}

template <typename TimeStepType, uint_t state_end, uint_t action_end>
ToyTextEnvBase<TimeStepType, state_end, action_end>::ToyTextEnvBase(
    const ToyTextEnvBase<TimeStepType, state_end, action_end> &other)
    : GymnasiumEnvBase<TimeStepType, ScalarDiscreteEnv<state_end, action_end>>(other)
{
}
...
ToyTextEnvBase<TimeStepType, state_end, action_end>::p(uint_t sidx, uint_t aidx) const
{
    ...
    auto response = this->get_api_server().dynamics(this->env_name(), this->idx(), sidx, aidx);
    return build_dynamics_from_response_(response);
}

template <typename TimeStepType, uint_t state_end, uint_t action_end>
typename ToyTextEnvBase<TimeStepType, state_end, action_end>::dynamics_t
ToyTextEnvBase<TimeStepType, state_end, action_end>::build_dynamics_from_response_(
