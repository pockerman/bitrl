...
/// exists in the world.
/// For a list of robots in webots checkout:
...
class WebotsEnvBase : public EnvBase<TimeStepType, SpaceType>
{
  ...
  protected:
    ...
    /// \brief Constructor
    ///
    WebotsEnvBase(const uint_t cidx, const std::string &name = rlenvscpp::consts::INVALID_STR);
};

template <typename TimeStepType, typename SpaceType>
WebotsEnvBase<TimeStepType, SpaceType>::WebotsEnvBase(const uint_t cidx, const std::string &name)
