...
EpuckSimpleGridWorld::EpuckSimpleGridWorld()
    : WebotsEnvBase<TimeStep<state_type>,
                    ContinuousVectorStateDiscreteActionEnv<3, // the state space has size 3
                                                           ...
                                                           real_t>>(static_cast<uint_t>(0),
                                                                    "EpuckSimpleGridWorld"),
      robot_()
{
}

void EpuckSimpleGridWorld::make(const std::string &version,
...
void EpuckSimpleGridWorld::build_make_options_(
    const std::unordered_map<std::string, std::any> &options)
{
...
void EpuckSimpleGridWorld::activate_robot_()
{

    auto &options_ = this->make_options();
    auto left_pos_sensor_time_step =
        this->template read_option<uint_t>("left_pos_sensor_time_step");
    robot_.activate_position_sensor(0, std::any_cast<uint_t>(left_pos_sensor_time_step));

    auto right_pos_sensor_time_step =
        this->template read_option<uint_t>("right_pos_sensor_time_step");
    ...
    auto right_motor_init_velocity =
        this->template read_option<real_t>("right_motor_init_velocity");
    robot_.activate_motor(1, std::any_cast<real_t>(right_motor_init_velocity));

    auto distance_sensor_time_step =
        this->template read_option<uint_t>("distance_sensor_time_step");
...
void EpuckSimpleGridWorld::close()
{
...
void EpuckSimpleGridWorld::pause_simulation() { robot_.pause_simulation(); }

real_t EpuckSimpleGridWorld::compute_reward() const
{
    ...
    else
    {
        ...
        if (min_dist_sensor_reading_goal > min_reading &&
            min_dist_sensor_reading_goal < max_reading)
        {
            return 10;
        }
...
EpuckSimpleGridWorld::time_step_type EpuckSimpleGridWorld::on_stop_()
{
...
EpuckSimpleGridWorld::time_step_type EpuckSimpleGridWorld::on_move_()
{
...
EpuckSimpleGridWorld::time_step_type EpuckSimpleGridWorld::step(const action_type &action)
{
