 ...
 */
template <typename TimeStepType, typename SpaceType> class EnvBase : public SpaceType
{
  public:
     ...
     */
    template <typename T> T read_option(const std::string &op_name) const;
...
EnvBase<TimeStepType, SpaceType>::EnvBase(const std::string &idx, const std::string &name)
    : SpaceType(), is_created_(false), idx_(idx), version_(), name_(name), current_state_()
{
}

template <typename TimeStepType, typename SpaceType>
EnvBase<TimeStepType, SpaceType>::EnvBase(const EnvBase<TimeStepType, SpaceType> &other)
    : SpaceType(), is_created_(other.is_created_), idx_(other.idx_), version_(other.version_),
      name_(other.name_), current_state_()
{
}

template <typename TimeStepType, typename SpaceType> void EnvBase<TimeStepType, SpaceType>::close()
{
...
template <typename TimeStepType, typename SpaceType>
template <typename T>
T EnvBase<TimeStepType, SpaceType>::read_option(const std::string &op_name) const
{
...
template <typename TimeStepType, typename SpaceType>
void EnvBase<TimeStepType, SpaceType>::make(
    const std::string &version, const std::unordered_map<std::string, std::any> &make_options,
