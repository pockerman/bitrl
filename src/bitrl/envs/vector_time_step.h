...
///
template <typename StateType> class VectorTimeStep
{
  ...
  public:
    ...
    ///
    VectorTimeStep(const std::vector<TimeStepTp> &types, const std::vector<real_t> &rewards,
    ...
    ///
    VectorTimeStep(const std::vector<TimeStepTp> &types, const std::vector<real_t> &rewards,
                   const std::vector<state_type> &obs, const std::vector<real_t> &discount_factors);
...
VectorTimeStep<StateType>::VectorTimeStep(const std::vector<TimeStepTp> &types,
                                          ...
                                          std::unordered_map<std::string, std::any> &&extra)
    : types_(types), rewards_(rewards), obs_(obs), discounts_(discount_factors), extra_(extra)
{
}
...
VectorTimeStep<StateType>::VectorTimeStep(const std::vector<TimeStepTp> &types,
                                          ...
                                          const std::vector<state_type> &obs,
                                          const std::vector<real_t> &discount_factors)
    : types_(types), rewards_(rewards), obs_(obs), discounts_(discount_factors)
{
}
...
VectorTimeStep<StateType>::VectorTimeStep(const std::vector<TimeStepTp> &types,
                                          const std::vector<real_t> &rewards,
                                          const std::vector<state_type> &obs)
    : VectorTimeStep<StateType>(types, rewards, obs, std::vector<real_t>())
{
}

template <typename StateType>
VectorTimeStep<StateType>::VectorTimeStep(const VectorTimeStep<StateType> &other)
    : types_(other.types_), rewards_(other.rewards_), obs_(other.obs_),
      discounts_(other.discounts_), extra_(other.extra_)
{
}
...
template <typename StateType> real_t VectorTimeStep<StateType>::reward() const noexcept
{
...
template <typename StateType> bool VectorTimeStep<StateType>::done() const noexcept
{
    auto done_ = false;
...
template <typename StateType> bool VectorTimeStep<StateType>::last() const noexcept
{
    return done();
}
