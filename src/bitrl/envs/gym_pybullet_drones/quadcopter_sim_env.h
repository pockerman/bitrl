...
{
  public:
    typedef typename EnvBase<TimeStep<std::vector<real_t>>, std::vector<real_t>>::time_step_type
        time_step_type;
    typedef typename EnvBase<TimeStep<std::vector<real_t>>, std::vector<real_t>>::action_type
        action_type;
    ...
    reset(uint_t seed, const std::unordered_map<std::string, std::any> &options) override final;
...
inline QuadcopterSimEnv::QuadcopterSimEnv(const std::string &url, const std::string &name)
    : EnvBase(url + "/gym-pybullet-drones/quadcopter-sim-env", name), current_state_()
{
}
